...
import {
  createCoachScheduleOverride,
  deleteCoachScheduleOverride,
  getCoachScheduleOverrideById,
  getCoachScheduleOverridePage,
  updateCoachScheduleOverride,
} from '@/servers/gym/coach-schedule-override';
...
function Page() {
  ...
  // 权限前缀
  const permissionPrefix = '/gym/coach-schedule-override';
  ...
  const getPage = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const { code, data } = await getCoachScheduleOverridePage(params);
  ...
  const onUpdate = async (id: string) => {
    try {
      ...
      const { code, data } = await getCoachScheduleOverrideById(id);
      if (Number(code) !== 200) return;
      setCreateData(data);
    } finally {
  ...
  const handleCreate = async (values: BaseFormData) => {
    try {
      ...
      const functions = () =>
        createId
          ? updateCoachScheduleOverride(createId, values)
          : createCoachScheduleOverride(values);
  ...
  const onDelete = async (id: string) => {
    try {
      setLoading(true);
      const { code, message } = await deleteCoachScheduleOverride(id);
      if (Number(code) === 200) {
