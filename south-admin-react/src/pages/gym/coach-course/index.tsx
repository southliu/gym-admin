...
function Page() {
  ...
  // 权限前缀
  const permissionPrefix = '/gym/coach-course';
  ...
  const getPage = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const { code, data } = await getCoachCoursePage(params);
  ...
  const handleCreate = async (values: BaseFormData) => {
    try {
      ...
      const { code, message } = await createCoachCourse(values);
      if (Number(code) !== 200) return;
  ...
  const onDelete = async (id: string) => {
    try {
      setLoading(true);
      const { code, message } = await deleteCoachCourse(id);
      if (Number(code) === 200) {
